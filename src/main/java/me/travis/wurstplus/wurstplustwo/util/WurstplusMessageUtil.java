...
public class WurstplusMessageUtil {
	...
	public static String opener = ChatFormatting.DARK_AQUA + Wurstplus.WURSTPLUS_NAME + ChatFormatting.RED + " | " + r;

	public static void toggle_message(WurstplusHack module) {
		if (module.is_active()) {
			if (module.get_tag().equals("AutoCrystal")) {
				client_message_simple(opener + "when" + ChatFormatting.DARK_GREEN + " boom boom ");
			} else {
				client_message_simple(opener + r + module.get_name() + ChatFormatting.DARK_GREEN + " Enabled");
			}			
		} else {
			if (module.get_tag().equals("AutoCrystal")) {
				client_message_simple(opener + "no more" + ChatFormatting.RED + " boom " + r + "boom");
			} else {
	...
	public static void send_client_message_simple(String message) {
		client_message(ChatFormatting.DARK_AQUA + "Huzuni+ 2 " + ChatFormatting.RED + "| " + r + message);
	}
