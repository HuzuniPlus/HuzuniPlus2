...
public class TurokBlockInteractionHelper
{
    ...
    public static void placeBlockScaffold(final BlockPos pos) {
        final Vec3d eyesPos = new Vec3d(mc.player.posX, mc.player.posY + Wrapper.getPlayer().getEyeHeight(), Wrapper.getPlayer().posZ);
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            if (canBeClicked(neighbor)) {
                ...
                if (eyesPos.squareDistanceTo(hitVec) <= 18.0625) {
                    ...
                    TurokBlockInteractionHelper.mc.rightClickDelayTimer = 4;
                    return;
                }
            }
        }
    ...
    private static void processRightClickBlock(final BlockPos pos, final EnumFacing side, final Vec3d hitVec) {
        getPlayerController().processRightClickBlock(Wrapper.getPlayer(), TurokBlockInteractionHelper.mc.world, pos, side, hitVec, EnumHand.MAIN_HAND);
    }
    ...
    public static double[] directionSpeed(final double speed) {
        float forward = TurokBlockInteractionHelper.mc.player.movementInput.moveForward;
        float side = TurokBlockInteractionHelper.mc.player.movementInput.moveStrafe;
        float yaw = TurokBlockInteractionHelper.mc.player.prevRotationYaw + (TurokBlockInteractionHelper.mc.player.rotationYaw - TurokBlockInteractionHelper.mc.player.prevRotationYaw) * TurokBlockInteractionHelper.mc.getRenderPartialTicks();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    static {
        TurokBlockInteractionHelper.mc = Minecraft.getMinecraft();
